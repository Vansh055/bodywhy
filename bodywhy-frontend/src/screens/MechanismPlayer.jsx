...
export function MechanismPlayer() {
  ...
  useEffect(() => {
    if (!id) return;
    getNode(id).then(setNode).catch(() => setNode(null));
    setBeat("hook");
  }, [id]);
  ...
  const stepIndex = beat.startsWith("step")
    ? Number(beat.replace("step", ""))
    : -1;
      ...
      <p
        ...
        style={{
          ...
          animation: reducedMotion
            ? "none"
            : "fadeSettle var(--duration-fast) var(--easing-standard)",
        }}
      >
        {beatText()}
      </p>
